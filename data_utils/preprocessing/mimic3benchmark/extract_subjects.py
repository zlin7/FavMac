# https://github.com/YerevaNN/mimic3-benchmarks/tree/master/mimic3benchmark/scripts
...
from data_utils.preprocessing.mimic3benchmark.mimic3csv import *
from data_utils.preprocessing.mimic3benchmark.util import dataframe_from_csv
...
#parser.add_argument('--phenotype_definitions', '-p', type=str, default=os.path.join(os.path.dirname(__file__), '../resources/hcup_ccs_2015_definitions.yaml'), help='YAML file with phenotype definitions.')
...
if isinstance(args.itemids_file, str):
	if os.path.isfile(args.itemids_file):
		items_to_keep = set([int(itemid) for itemid in dataframe_from_csv(args.itemids_file)['ITEMID'].unique()]) 
	else:
		items_to_keep = args.itemids_file.split(",")
else:
	items_to_keep = None
