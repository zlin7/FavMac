...
def train(
    ...
    lr=1e-2, n_epochs=50, batch_size=128, continue_from_key=None,
    train_split=data_utils.TRAIN, val_split=data_utils.VALID, eval_steps=200,
    model_class = models.MNISTCNN, model_kwargs=None, gpu_id=0,
    ...
    **kwargs):
    ...
    if eval_datakwargs is not None:
        if isinstance(gpu_id, tuple) or isinstance(gpu_id, list):
            gpu_id = gpu_id[0]
        for split in [val_split, 'val1', data_utils.TEST]:
...
class _Configs:
    @classmethod
    def train(cls, task_runner:utils.TaskPartitioner = None, debug=False, gpu_id=(0, 1)):
        if task_runner is None: task_runner = utils.TaskPartitioner()
        dataset = _settings.MNISTSup_NAME
        for sample_proba in [0.4]:
            lr = 1e-2
            kwargs = {"datakwargs": {'sample_proba': sample_proba, 'noise_level': None}, 'train_split': data_utils.TRAIN}
            kwargs['model_kwargs'] = {'size': 48}
...
if __name__ == '__main__':
    debug = False
    res = _Configs.train(debug=False).run()
    print(res[0][0])
    print(res[1][0])